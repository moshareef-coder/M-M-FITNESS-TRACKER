...
## Resolved: "returning after a break," and two related product decisions

**Update:** the modifier-vs-bubble question below is no longer open — found
direct evidence in `index.html`'s `RETIRED_GOALS` migration log. Mo tried
"get-back" as its own first-class bubble, then retired it, migrating it
into "consistent." His resolution lands closer to this file's original
modifier framing than his research's "own bubble" argument, though not
identical — it defaults into the general bucket rather than applying as a
true cross-cutting layer on top of any goal. Worth treating his actual
shipped decision as authoritative going forward rather than re-litigating
the two readings below.

Two more retirements from the same log, both directly relevant here:

- **"do-a-thing" (first pull-up, first push-up) → retired, migrated to
  get-stronger.** Mo's own comment: *"A first pull-up and a first push-up
  are bodyweight strength, and 'Lift my bodyweight' is get-stronger's own
  word for that."* Matches this file's Skill ladder category conclusion
  independently.
- **"feel-better" → retired 2026-09-17, migrated to consistent.** Mo's own
  reasoning: *"we are a workout app"* — sleep, energy, and mood are real,
  but the app doesn't program for them distinctly, so a separate tile
  promised something the plan never delivered. This directly overrides
  this file's Cluster 8 recommendation to keep "feel better" and "keep
  quitting" as two distinct framings — Mo's call is that the app shouldn't
  offer a tile it can't back with differentiated programming, which is a
  reasonable product boundary this research didn't have visibility into
  when it was written.
...
being buried as a follow-up modifier under another goal. See the
resolution above — kept here for the reasoning trail, not as an open item.
