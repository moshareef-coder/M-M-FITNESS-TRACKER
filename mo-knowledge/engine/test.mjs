...
test("a class is built for the mat mileage they have, not for how much they lift", async () => {
  /* plan.level is a lifting training age. An advanced deadlifter who has never
     been on a mat is a beginner here, and the only honest input is how many of
     these they have done, which the app writes into exercise_logs under the
     activity's own label. */
  const lifter = await generateFromPayload(stylePayload(["yoga"], { logs: manySessions(80) }));
  assert.notEqual(lifter.meta.level, "beginner", "the lifting age really is past beginner");
...
test("a kind we cannot build falls through to one we can, rather than to a refusal", async () => {
  /* The cardio library's only swim is tagged intermediate, so a beginner who
     ...
     honest answer to that is the yoga class they also asked for, and not the
     lifting day they did not. */
  const out = await generateFromPayload(stylePayload(["swimming", "yoga"]));
  assert.equal(out.meta.styles.honoured, true);
  assert.equal(out.workout.flow?.training, "yoga");
  assert.equal(out.workout.exercises.length, 0);
});
...
test("a week we cannot build still says so in a sentence, and does not claim otherwise", async () => {
  ...
  for (const styles of [["sports"], ["swimming"], ["classes"]]) {
    const out = await generateFromPayload(stylePayload(styles));
