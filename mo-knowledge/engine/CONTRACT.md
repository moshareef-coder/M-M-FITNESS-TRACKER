...
| `session_minutes` | number | engine | **new.** How long one session should take, in minutes. Absent, `null`, `0` and nonsense all mean "never answered" and the goal's own session length runs. A real answer replaces it: the week is built to fit the number, in both directions. Clamped to 15 and 120, and the clamp is said in `notes`. **2026-09-12**: a longer answer than the plan needs buys two things that cost no recovery: the rest the clock took back, if a lighter week left room for it, and a ten minute stretching block instead of five. It never buys hard sets past the weekly ceiling, and when there is nothing left to buy it still says so. Ramp sets are NOT among them: they are warm-up, they are on every day that has something to ramp, and no session length decides them **2026-09-19, the clock is honest.** The floor is 25, not 15: it is the smallest full day the engine really builds (four movements at two sets, the shortest rest, the warm-up and the cool-down), measured, and an ask under it is refused out loud rather than built for 25 with a note claiming it fits: every full day of such a plan is in `volumeNotes.overBudget`, `session.fits` is false and the sentence says "You asked for 20 minutes. There is no session that short". Every trim measures the whole visit against the BUILT blocks (the warm-up runs six to seven minutes, the cool-down five to six, and the app costs what was built) and allows three minutes over, not fifteen percent; feeler sets on lifts with no weight yet are costed too (see `session.feelerMinutes`). Measured before and after on 1,100 runs (11 bubbles x 5 day counts x 4 histories x 5 clocks): days five or more minutes over the ask went 67.6% to 33.6%, and days over by that much while claiming `fits: true` went 1,559 to 0 |
| `train_styles` | text[] | engine | **new.** What the person agreed to do, from onboarding: `lifting`, `home`, `running`, `cycling`, `walking`, `pilates`, `yoga`, and nothing else. **2026-09-19**: `swimming`, `rowing`, `classes`, `hiking` and `sports` were removed by product decision, each having been a place the engine had to refuse; a stored one is dropped like a typo, so a profile holding only removed values reads as "never asked". Strict opt in by product decision: a style not in the list is never planned and there is no floor putting resistance training back. `null`, an empty list and a list of nothing recognisable all mean "never asked" and build exactly the plan built before this column existed. Two resistance styles. `lifting` means a gym, and **2026-09-21** it finally does something: it narrows nothing, which is why it did nothing at all until that date, so it now DECLARES instead. What somebody says they have reorders the movements a slot may pick, so the slot takes the simplest movement that uses their kit rather than the simplest movement in the library, and a new gym member stops opening on push-ups. It changes no eligibility: what a person may be shown is still the safe default pool plus what they earned, and both hard filters, `limits.missing` and the named joint, come first. `home` without `lifting` means bodyweight and dumbbells only and folds into the equipment limit rather than opening a second path through the builder; it declares BODYWEIGHT ALONE, because the tile reads "Without equipment" and the dumbbell is a fallback the plan may reach for, not a rack they told us about, so a `home` week is byte for byte the week it was before this existed. A limit set by hand on the limits sheet survives the merge. A person who ticked no resistance style gets the day they asked for: a cardio session built from `knowledge/exercise-library/cardio.mjs` (see `workout.cardio`), or a yoga or Pilates class built from those libraries (see `workout.flow`), alternating by date when they ticked both kinds. Where nothing in what they ticked can be built, which no tick on the seven style sheet reaches with the library as it ships, the lifting day stands and `meta.styles.honoured` is false with a sentence saying so |
| `style_frequency` | object | engine | **new 2026-09-21.** How often each ticked style, keyed by the same ids `train_styles` holds, one of `daily`, `most`, `some`, `rare` (every day, most days, sometimes, rarely; 7, 5, 3 and 1 days of a seven day week). Read by `readStyles(train_styles, style_frequency)` in styles.mjs and answered for EVERY picked style: a style the map does not name gets the week it already had, which is `most` for a resistance style (or for the first pick when nothing resistance was ticked) and `rare` for everything else. `null`, `{}` and a map of words the engine does not know all mean "never asked" and change nothing, the same promise `train_styles` makes for null. What it changes: the single day this function builds for a no-lifting week is weighted by it (`styleDayFor` puts a kind in its ring once per day its word is worth, so running most days and yoga rarely is five runs in six), and only once it was really answered, so a legacy profile keeps the plain alternation. The WEEK is composed by the app (index.html `generateTheWeek`) through `composeWeek(styles, days, {liftCount, window, seed})`, exported from styles.mjs and under the gate: a day holds at most one resistance session, one cardio session and one class, never two of a kind; `liftCount` is `days_per_week`, which outranks the resistance word because it is the exact number; a style due once lands on the quietest day, cardio keeps off the leg day where another day will do, flow prefers the day after the heaviest lift, and what could not fit is returned under `dropped` rather than lost. The answer is echoed at `meta.styles.frequency` |
...
| `swaps` | array | engine | `exercise_swaps` rows, `{ entry_date, planned_exercise, chosen_exercise }`. What somebody reached for instead. **Live as of the same date and for the same reason.** With `plans`, it feeds `preferences.mjs`: two occurrences inside 90 days sink a movement to the bottom of its pool, which takes it off the card whenever the pool holds anything else, and the plan says so by name and says it is one step down rather than out; three take it out of the plan and the plan says so by name. Neither ever happens in silence (measured 2026-09-19: the sink was removing at two with nothing said). It reorders the candidates for a slot and nothing else. It never changes a set count, a rep range, a load or the split, and it never leaves a slot empty |
...
```
{
  workout: {
    ...
    exercises: [
      {
        ...
        rotatedFor: "Lat Pulldown",     // new 2026-09-21, and only present on a lift standing in for a stalled one. Same treatment: store it, the engine reads it back off `plans` and holds the swap for a block rather than for one week
        rotatedAt: "2026-06-08"         // with it, the day of the first swap, carried forward on every later row so the block is measured from the swap and not from whichever row is still inside the thirty day window
      }
    ],
    ...
    rampSets: [                         // new 2026-09-12, OFF as of 2026-09-20 (see below): absent on every response until it is turned back on
      {
...
**Off for now, 2026-09-20.** The owner's call: "get rid of the ramp feature for
now completely." `rampsForDay` in plan.mjs returns none for every day of every
plan, so `rampSets` is **absent on every response**, `session.rampMinutes` is
always `0`, `stretching.warmupMinutes` never takes the shorter ramped branch,
and `age.warmupApplied`'s own ramped-day comparison never has a ramped day to
find. Everything below this line still describes the mechanism correctly: it
is the reasoning `rampsForDay` stopped calling, still in plan.mjs, unreached
rather than deleted, and turning it back on is un-doing that one function
rather than re-deriving the load-safety case for it. Read this section as
"what a ramp is when the feature is on," not as the current response shape.
...
left the engine. Render it beside `honest` on the plan card (the reveal is gone, 2026-09-19); nothing needs storing,
...
| `deload` | object\|null | **new 2026-09-19.** `{ everyWeeks, detail, week, trainedWeeks }`, or `null` for somebody still on linear progression (the schedule and the progression rule read the same two measured facts, so they never disagree). `week` is whether THIS week is the deload: the engine builds it, about two thirds of the sets and a tenth off every load that has one, same exercises, with the sentence in `notes`. `trainedWeeks` is how many calendar weeks in a row before this one carry a completed plan, counted off `plans`; a week with nothing completed resets it (a week off is a deload), and so does a row carrying `deload: true`, which a client may write onto the stored plan so the count survives a screen that shows history. The current week is never counted, so a rebuild on Thursday agrees with Monday's |
| `progression` | object | **new 2026-09-19.** `{ rule, detail }`: `linear` (add weight when every rep lands) or `double` (top of the rep range, then add weight and drop back). Decided from the bar, research/04: double only where linear has measurably stopped working |
| `volume` | object | **new 2026-09-19.** `{ byGroup, under, frequencyCapped }`. `byGroup` is the weekly ledger, one `{ sets, target, wanted }` per muscle group the week trains (obliques count under `abs`, the one row the research has). `under` lists groups short of their target with an accessory slot that has room, `frequencyCapped` the groups this split cannot train often enough to reach what the week asked for; both are objects with a `why` sentence each, and the sentences already reach `notes` |
| `rotation` | object | **new 2026-09-21.** `{ block, anchors, held, accessories }`: what moved this week and why. `anchors` are stalled lifts rotated out this week, `{ from, to, day }`; `held` are stand-ins still in their block, `{ from, to, since, weeksLeft }` (`weeksLeft` 0 once the block is over and the stand-in stays only while it keeps going up); `accessories` are the calendar turns, `{ day, name, insteadOf }`. `block` is the calendar fortnight, 0 for a person with nothing on the record, whose week is the ranked answer. The sentences already reach `notes` |
...
| `age.rampApplied` | bool | **new.** `true` whenever `rampCaution` is above 0, which is every plan built without a known age and every plan for somebody over 30. plan.mjs has read `person.ageCaution` since 2026-09-12, at the destructure, the `calibrate` call and the `prescribeLoad` call. This row said "false today, plan.mjs does not read it yet" until 2026-09-19, a week after that stopped being true |
