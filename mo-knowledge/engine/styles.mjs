...
/* And what to say when we could NOT build that week and are handing back a
   ...
   library we have; a cardio mode with nothing at this person's level; a flow
...
/* How hard a class to build, from how many of these they have already done.
   ...
   Not from `plan.level`. That is a lifting training age, worked out of logged
   loads and dated sessions, and an advanced deadlifter who has never been on a
   mat is a beginner here. activity-session.mjs is explicit that the only
   honest input is mileage, and mileage is what the payload carries: the app
   writes the activity's own label into exercise_logs.exercise_name, so "Yoga"
   rows are yoga sessions and they can be counted.
...
/* THE DAY a week with no resistance training in it should get, which is a real
   ...
   substitution. cardioSessionFor refuses a mode the library cannot serve at
   this person's level, and for somebody who also ticked Yoga the honest answer
...
export function styleDayFor(styles, { level = "beginner", today = new Date(), minutes = null, askedMinutes = null, logs = [] } = {}) {
  const ring = [];
  ...
  for (let i = 0; i < ring.length; i++) {
    ...
    const day = pick === "cardio"
      ? cardioSessionFor(styles, { level, today, minutes })
