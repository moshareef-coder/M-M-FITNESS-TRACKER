...
export function generateFromPayload(rawPayload = {}, { today = new Date(), includePlan = false } = {}) {
  ...
  try {
    ...
    const styleSession = styles.asked && !styles.resistance
      ? styleDayFor(styles, {
        /* No `level`. The engine no longer carries a training level for a
           person, and the cardio library is asked on the strength of the mode
           they ticked rather than a label about them. See styles.mjs. */
        today: forDate, logs,
